...
class LocalTest:
    ...
    def _parse_sql(self, sql):
        try:
            ...
            if not isinstance(parsed, valid):
                self.console.print(
                    f"[yellow]Warning: unusual statement type {type(parsed).__name__}[/yellow]"
                )
            ...
            self.console.print(
                Panel(str(err), title="Unexpected Error", border_style="red")
            )
            return
        ...
        prett_sql = transpile(sql, read=self.db_system, pretty=True)[0]

        self.console.print(Syntax(prett_sql, "sql", theme="monokai"))
    ...
    def test_template_query(self, iac_action: str):
        ...
        drift = Drift(adapter=BaseAdapter)

        rsc_definition = drift._check_state_values(
            state={},
        ...
        sql = utils.render_templates(
            ...
            definition=rsc_definition.template_args,
            name=self.resource_name,
            iac_action=self.db_sys_resources[self.resource_type]["iac_action"][
                iac_action
            ],
        )
